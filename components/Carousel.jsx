...
export default function ProgressSlider({ items }) {
  ...
  return (
    <div className="w-full  mx-auto text-center">
      ...
      {/* Buttons */}
      <div className="min-w-full sm:max-w-sm md:max-w-3xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 bg-[#EE2326] px-3 py-5">
        {items.map((item, index) => (
          <button
            key={index}
          ...
          >
            ...
            >
              <span className="flex items-center justify-center relative w-9 h-9 rounded-full bg-indigo-50 mb-2">
                <Image
                  src={item.buttonIcon}
                  alt={item.desc}
                  width={40}
              ...
              <span className="block text-sm font-semibold  font-inter text-slate-100 mb-2">
                {item.desc}
              </span>
              <span
              ...
              >
                <span
                  className="absolute inset-0 bg-slate-400 rounded-[inherit]"
